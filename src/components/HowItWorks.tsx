...
const STEPS = [
  {
    ...
    icon: (
      <svg className="w-8 h-8 text-sage/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <Section id="process" className="bg-white py-24 md:py-32 border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="text-center mb-20 space-y-4">
          ...
          <div className="sage-rule sage-rule-center mt-6" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
          {STEPS.map((step, index) => (
            ...
            >
              ...
              {index < 2 && (
                <div className="hidden md:block absolute -right-6 top-12 bottom-12 w-[1px] bg-sage/20" />
              )}
              
              <div className="space-y-2">
                <span className="font-display text-5xl text-sage/40 block">{step.number}</span>
