...
export default function FAQ() {
  ...
  return (
    <Section id="faq" className="bg-sage-light py-24 md:py-32">
        ...
        <div className="space-y-4">
          {FAQ_ITEMS.map((item, index) => (
            ...
            >
              ...
              >
                <span className="font-display text-xl md:text-2xl text-charcoal group-hover:text-sage transition-colors">
                  {item.question}
                </span>
                {openIndex === index ? (
                  <Minus className="w-5 h-5 text-sage" />
                ) : (
                  <Plus className="w-5 h-5 text-sage group-hover:scale-110 transition-transform" />
                )}
              </button>
